...
| Board size | 77.2 × 82.4 mm, single up |
| Thickness | 1.6 mm |
...
| --- | --- | --- |
| Min track width | 0.30 mm | 0.09–0.127 mm |
| Power track width | 0.80 mm | n/a |
| Min clearance | 0.25 mm | 0.09–0.127 mm |
| Min drill | 0.40 mm | 0.20–0.30 mm |
| Via pad / drill | 0.80 / 0.40 mm | n/a |
...
Their argument is not about current: it is that a fabricator's cumulative drill
and layer-registration error runs to about ±0.003" (0.076 mm), and a layout
that has not budgeted for it is relying on the contractor being better than
they promised. A 0.20 mm annular ring absorbs the full error and still leaves
0.124 mm, against an IPC-2221 Class 2 minimum of 0.05 mm; the old 0.15 mm ring
left 0.074 mm. The 0.40 mm drill takes the aspect ratio through 1.6 mm of board
from 5.3:1 to 4:1.

**Power and ground vias are doubled** wherever there was room — the ten bypass
capacitors and the V+ taps — which is why the via count rose by more than the
capacitor count would explain. They are not doubled under the SOIC-14 and SO-14
packages, where the pad columns are 3.0 mm apart and the pins 1.27 mm, nor on
the two switch supply taps. That is RMC's own "where it is practical to do so".

One declared shortfall: RMC asked for 0.010" clearance and 0.25 mm is 0.0098".
Four microns under, kept because the whole layout is on a metric grid.
...
**This has been put to RMC three times and not yet confirmed.** Do not order
until it is.
